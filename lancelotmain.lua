...
do
    ...
    local API_BASE = "https://uhqhub-production.up.railway.app"
    ...
    local function checkKey(key)
        ...
        local ok, result = pcall(function()
            return game:HttpGet(API_BASE .. "/check?key=" .. key .. "&hwid=" .. hwid)
        end)
        if not ok then return false, "Erreur réseau" end
        if result == "valid" then return true, "OK"
        elseif result == "hwid_mismatch" then return false, "HWID différent — contacte le support"
        elseif result == "invalid" then return false, "Key invalide"
        else return false, "Erreur inconnue" end
    end

    local key = _G.LANCELOT_KEY
    if not key or key == "" then
        error("❌ Aucune key fournie. Mets ta key dans _G.LANCELOT_KEY avant d'exécuter.")
    end
...
local function kickPlayer()
    LocalPlayer:Kick("\ndiscord.gg/lancelothub - xi loves you <3")
end
...
task.spawn(function()
    ...
    -- ============================================================
    -- AUTO BUY BRAINROT PANEL
    -- ============================================================
    do
        -- Cache des brainrots libres trouvés dans le monde
        SharedState.WorldBrainrots = {} -- {uid, name, mutation, prompt, part}
        SharedState.AutoBuySelectedUID = nil

        local function scanWorldBrainrots()
            local plots = Workspace:FindFirstChild("Plots")
            local found = {} -- uid -> entry
            for _, obj in ipairs(Workspace:GetDescendants()) do
                if plots and obj:IsDescendantOf(plots) then continue end
                if not (obj:IsA("ProximityPrompt") and obj.Enabled) then continue end
                -- Le prompt doit avoir ActionText == "Steal" comme les autres brainrots
                -- OU pas d'ActionText connu → on prend tout hors Plots et on filtre les NPCs
                -- Un brainrot libre a toujours un modèle avec "AnimalOverhead" ou un attribut "Index"
                local part = obj.Parent
                if not part then continue end
                local model = part:IsA("Model") and part
                    or (part.Parent and part.Parent:IsA("Model") and part.Parent)
                    or nil
                if not model then continue end

                -- Chercher le nom via attribut "Index" (comme AnimalsData) ou AnimalOverhead
                local indexAttr = model:GetAttribute("Index") or model:GetAttribute("AnimalIndex")
                local nameFound = nil
                local mutFound = nil

                if indexAttr then
                    nameFound = tostring(indexAttr)
                end

                -- Chercher via AnimalOverhead BillboardGui
                for _, d in ipairs(model:GetDescendants()) do
                    if d:IsA("BillboardGui") and (d.Name == "AnimalOverhead" or d.Name:find("Overhead")) then
                        local lbl = d:FindFirstChildWhichIsA("TextLabel", true)
                        if lbl and lbl.Text and lbl.Text ~= "" then
                            nameFound = nameFound or lbl.Text
                        end
                    end
                    if d:IsA("StringValue") and (d.Name == "Index" or d.Name == "AnimalIndex") then
                        nameFound = nameFound or d.Value
                    end
                end

                -- Chercher mutation
                mutFound = model:GetAttribute("Mutation") or model:GetAttribute("mutation")
                if mutFound == "None" or mutFound == "" then mutFound = nil end

                -- Si pas de nom trouvé, utiliser le nom du modèle comme fallback
                if not nameFound then nameFound = model.Name end

                -- Ignorer les objets workspace génériques
                if nameFound == "Workspace" or nameFound == "Model" or nameFound == "" then continue end

                local uid = tostring(model)
                local basePart = part:IsA("BasePart") and part
                    or (part:IsA("Attachment") and part.Parent:IsA("BasePart") and part.Parent)
                    or model:FindFirstChildWhichIsA("BasePart", true)

                found[uid] = {uid=uid, name=nameFound, mutation=mutFound, prompt=obj, part=basePart, model=model}
            end
            -- Convertir en tableau
            local result = {}
            for _, v in pairs(found) do table.insert(result, v) end
            SharedState.WorldBrainrots = result
            return result
        end

        local autoBuyGui = Instance.new("ScreenGui")
        autoBuyGui.Name = "AutoBuyUI"; autoBuyGui.ResetOnSpawn = false; autoBuyGui.Parent = PlayerGui

        local abFrame = Instance.new("Frame")
        local abMobileScale = IS_MOBILE and 0.6 or 1
        abFrame.Size = UDim2.new(0, 300*abMobileScale, 0, 500*abMobileScale)
        abFrame.Position = UDim2.new(0.02, 0, 0.55, 0)
        abFrame.BackgroundColor3 = Theme.Background; abFrame.BackgroundTransparency = 0.05
        abFrame.BorderSizePixel = 0; abFrame.ClipsDescendants = true; abFrame.Parent = autoBuyGui
        Instance.new("UICorner", abFrame).CornerRadius = UDim.new(0, 12)
        local abStroke = Instance.new("UIStroke", abFrame)
        abStroke.Color = Theme.Accent1; abStroke.Thickness = 1.5; abStroke.Transparency = 0.4

        local abHeader = Instance.new("Frame", abFrame)
        abHeader.Size = UDim2.new(1,0,0,40); abHeader.BackgroundTransparency = 1
        MakeDraggable(abHeader, abFrame, "AutoBuy")

        local abTitle = Instance.new("TextLabel", abHeader)
        abTitle.Size = UDim2.new(1,-15,1,0); abTitle.Position = UDim2.new(0,15,0,0)
        abTitle.BackgroundTransparency = 1; abTitle.Text = "AUTO BUY"
        abTitle.Font = Enum.Font.GothamBlack; abTitle.TextSize = 16
        abTitle.TextColor3 = Theme.TextPrimary; abTitle.TextXAlignment = Enum.TextXAlignment.Left

        -- Cible actuelle
        local abTargetPanel = Instance.new("Frame", abFrame)
        abTargetPanel.Size = UDim2.new(1,-30,0,42); abTargetPanel.Position = UDim2.new(0,15,0,45)
        abTargetPanel.BackgroundColor3 = Theme.Surface; abTargetPanel.BorderSizePixel = 0
        Instance.new("UICorner", abTargetPanel).CornerRadius = UDim.new(0, 8)
        local abTargetHeader = Instance.new("TextLabel", abTargetPanel)
        abTargetHeader.Size = UDim2.new(1,-10,0,14); abTargetHeader.Position = UDim2.new(0,8,0,5)
        abTargetHeader.BackgroundTransparency = 1; abTargetHeader.Text = "TARGET"
        abTargetHeader.Font = Enum.Font.GothamBold; abTargetHeader.TextSize = 10
        abTargetHeader.TextColor3 = Theme.TextSecondary; abTargetHeader.TextXAlignment = Enum.TextXAlignment.Left
        local abTargetLabel = Instance.new("TextLabel", abTargetPanel)
        abTargetLabel.Size = UDim2.new(1,-10,0,18); abTargetLabel.Position = UDim2.new(0,8,0,20)
        abTargetLabel.BackgroundTransparency = 1; abTargetLabel.Font = Enum.Font.GothamBold; abTargetLabel.TextSize = 14
        abTargetLabel.TextColor3 = Theme.TextPrimary; abTargetLabel.TextXAlignment = Enum.TextXAlignment.Left
        abTargetLabel.TextTruncate = Enum.TextTruncate.AtEnd; abTargetLabel.Text = "ANY"

        -- Bouton ON/OFF
        local abEnableBtn = Instance.new("TextButton", abFrame)
        abEnableBtn.Size = UDim2.new(1,-30,0,28); abEnableBtn.Position = UDim2.new(0,15,0,95)
        abEnableBtn.BackgroundColor3 = Config.AutoBuyBrainrot and Theme.Success or Theme.SurfaceHighlight
        abEnableBtn.Text = Config.AutoBuyBrainrot and "ENABLED" or "DISABLED"
        abEnableBtn.Font = Enum.Font.GothamBold; abEnableBtn.TextSize = 13
        abEnableBtn.TextColor3 = Theme.TextPrimary
        Instance.new("UICorner", abEnableBtn).CornerRadius = UDim.new(0, 8)
        abEnableBtn.MouseButton1Click:Connect(function()
            Config.AutoBuyBrainrot = not Config.AutoBuyBrainrot
            SaveConfig()
            abEnableBtn.Text = Config.AutoBuyBrainrot and "ENABLED" or "DISABLED"
            abEnableBtn.BackgroundColor3 = Config.AutoBuyBrainrot and Theme.Success or Theme.SurfaceHighlight
            ShowNotification("AUTO-BUY BRAINROT", Config.AutoBuyBrainrot and "ENABLED" or "DISABLED")
        end)

        -- Label liste
        local abSelectLabel = Instance.new("TextLabel", abFrame)
        abSelectLabel.Size = UDim2.new(1,-30,0,18); abSelectLabel.Position = UDim2.new(0,15,0,130)
        abSelectLabel.BackgroundTransparency = 1; abSelectLabel.Text = "BRAINROTS DISPONIBLES"
        abSelectLabel.Font = Enum.Font.GothamBold; abSelectLabel.TextSize = 11
        abSelectLabel.TextColor3 = Theme.TextSecondary; abSelectLabel.TextXAlignment = Enum.TextXAlignment.Left

        -- Liste scrollable
        local abListFrame = Instance.new("ScrollingFrame", abFrame)
        abListFrame.Size = UDim2.new(1,-30,1,-155); abListFrame.Position = UDim2.new(0,15,0,150)
        abListFrame.BackgroundTransparency = 1; abListFrame.BorderSizePixel = 0
        abListFrame.ScrollingDirection = Enum.ScrollingDirection.Y
        abListFrame.ScrollBarImageTransparency = 1; abListFrame.ScrollBarThickness = 0
        local abLayout = Instance.new("UIListLayout", abListFrame)
        abLayout.Padding = UDim.new(0,6); abLayout.SortOrder = Enum.SortOrder.LayoutOrder
        abLayout:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function()
            abListFrame.CanvasSize = UDim2.new(0,0,0, math.max(0, abLayout.AbsoluteContentSize.Y))
        end)
        ...
        local MUT_COLORS_AB = {
            ...
            Rainbow=Color3.fromRGB(255,100,200), Lava=Color3.fromRGB(255,100,20),
            Candy=Color3.fromRGB(255,105,180), Divine=Color3.fromRGB(255,255,255),
            Bloodrot=Color3.fromRGB(139,0,0), Radioactive=Color3.fromRGB(0,255,0)
        }

        -- Bouton ANY
        local anyBtn = Instance.new("TextButton", abListFrame)
        anyBtn.Size = UDim2.new(1,0,0,32); anyBtn.LayoutOrder = 0
        anyBtn.BackgroundColor3 = Theme.Accent1
        anyBtn.Text = "★ ANY BRAINROT"; anyBtn.Font = Enum.Font.GothamBold
        anyBtn.TextSize = 12; anyBtn.TextColor3 = Color3.new(0,0,0)
        Instance.new("UICorner", anyBtn).CornerRadius = UDim.new(0,6)

        local function refreshAutoBuyList()
            for _, b in ipairs(abButtons) do pcall(function() b:Destroy() end) end
            abButtons = {}

            local entries = scanWorldBrainrots()

            if #entries == 0 then
                -- Afficher message si rien trouvé
                local emptyLbl = Instance.new("TextButton", abListFrame)
                emptyLbl.Size = UDim2.new(1,0,0,32); emptyLbl.LayoutOrder = 1
                emptyLbl.BackgroundColor3 = Theme.Surface; emptyLbl.Text = "Aucun brainrot détecté"
                emptyLbl.Font = Enum.Font.GothamBold; emptyLbl.TextSize = 11
                emptyLbl.TextColor3 = Theme.TextSecondary
                Instance.new("UICorner", emptyLbl).CornerRadius = UDim.new(0,6)
                table.insert(abButtons, emptyLbl)
            else
                for i, entry in ipairs(entries) do
                    local btn = Instance.new("TextButton", abListFrame)
                    btn.Size = UDim2.new(1,0,0,36); btn.BackgroundColor3 = Theme.Surface
                    btn.Text = ""; btn.LayoutOrder = i
                    Instance.new("UICorner", btn).CornerRadius = UDim.new(0,6)
                    local isSelected = SharedState.AutoBuySelectedUID == entry.uid
                    local hasMut = entry.mutation and entry.mutation ~= "None" and entry.mutation ~= ""
                    local col = hasMut and (MUT_COLORS_AB[entry.mutation] or Color3.fromRGB(210,130,255)) or Theme.Accent2
                    local bStroke = Instance.new("UIStroke", btn)
                    bStroke.Color = col; bStroke.Thickness = 1; bStroke.Transparency = isSelected and 0 or 1
                    btn.BackgroundColor3 = isSelected and Theme.SurfaceHighlight or Theme.Surface

                    local itemBar = Instance.new("Frame", btn)
                    itemBar.Size = UDim2.new(0,3,1,-8); itemBar.Position = UDim2.new(0,3,0,4)
                    itemBar.BackgroundColor3 = col; itemBar.BorderSizePixel = 0
                    Instance.new("UICorner", itemBar).CornerRadius = UDim.new(1,0)
                    ...
                    rankLabel.BackgroundTransparency = 1; rankLabel.Text = "#"..i
                    rankLabel.Font = Enum.Font.GothamBlack; rankLabel.TextSize = 13
                    rankLabel.TextColor3 = isSelected and Theme.Accent1 or Theme.TextSecondary

                    local infoLabel = Instance.new("TextLabel", btn)
                    infoLabel.Size = UDim2.new(1,-42,1,0); infoLabel.Position = UDim2.new(0,38,0,0)
                    infoLabel.BackgroundTransparency = 1; infoLabel.RichText = true
                    local mutText = hasMut and formatMutationText(entry.mutation) or ""
                    infoLabel.Text = mutText.."<font weight='700'>"..entry.name.."</font>"
                    infoLabel.Font = Enum.Font.GothamMedium; infoLabel.TextSize = 12
                    infoLabel.TextXAlignment = Enum.TextXAlignment.Left; infoLabel.TextTruncate = Enum.TextTruncate.AtEnd
                    infoLabel.TextColor3 = isSelected and Theme.TextPrimary or Theme.TextSecondary

                    btn.MouseButton1Click:Connect(function()
                        SharedState.AutoBuySelectedUID = entry.uid
                        abTargetLabel.Text = entry.name
                        anyBtn.BackgroundColor3 = Theme.Surface
                        anyBtn.TextColor3 = Theme.TextPrimary
                        ShowNotification("AUTO-BUY TARGET", entry.name)
                        refreshAutoBuyList()
                    end)
                    table.insert(abButtons, btn)
                end
            end

            -- Mettre à jour bouton ANY
            anyBtn.BackgroundColor3 = SharedState.AutoBuySelectedUID == nil and Theme.Accent1 or Theme.Surface
            anyBtn.TextColor3 = SharedState.AutoBuySelectedUID == nil and Color3.new(0,0,0) or Theme.TextPrimary
            abListFrame.CanvasSize = UDim2.new(0,0,0, math.max(0, abLayout.AbsoluteContentSize.Y))
        end

        anyBtn.MouseButton1Click:Connect(function()
            SharedState.AutoBuySelectedUID = nil
            abTargetLabel.Text = "ANY"
            refreshAutoBuyList()
            ShowNotification("AUTO-BUY TARGET", "ANY BRAINROT")
        end)

        -- Refresh toutes les 2 secondes
        task.spawn(function()
            while autoBuyGui.Parent do
                task.wait(2)
                pcall(refreshAutoBuyList)
            end
        end)
        task.delay(1.5, function() pcall(refreshAutoBuyList) end)
    end
    -- ============================================================
...
local rAutoBuyBrainrot = CreateRow("Auto-Buy Brainrot")
CreateToggleSwitch(rAutoBuyBrainrot, Config.AutoBuyBrainrot, function(ns, set)
    set(ns); Config.AutoBuyBrainrot = ns; SaveConfig()
    ShowNotification("AUTO-BUY BRAINROT", ns and "ENABLED" or "DISABLED")
end)
local bAutoBuyKey = Instance.new("TextButton", rAutoBuyBrainrot)
bAutoBuyKey.Size = UDim2.new(0, 60, 0, 24)
bAutoBuyKey.Position = UDim2.new(1, -140, 0.5, -12)
bAutoBuyKey.BackgroundColor3 = Theme.SurfaceHighlight
bAutoBuyKey.Text = Config.AutoBuyBrainrotKey ~= "" and Config.AutoBuyBrainrotKey or "NONE"
bAutoBuyKey.Font = Enum.Font.GothamBold
bAutoBuyKey.TextColor3 = Theme.TextPrimary
bAutoBuyKey.TextSize = 12
Instance.new("UICorner", bAutoBuyKey).CornerRadius = UDim.new(0, 4)
bAutoBuyKey.MouseButton1Click:Connect(function()
    bAutoBuyKey.Text = "..."; bAutoBuyKey.TextColor3 = Theme.Accent1
    local con; con = UserInputService.InputBegan:Connect(function(inp, gpe)
        if gpe then return end
        Config.AutoBuyBrainrotKey = inp.KeyCode.Name
        bAutoBuyKey.Text = inp.KeyCode.Name
        bAutoBuyKey.TextColor3 = Theme.TextPrimary
        SaveConfig(); con:Disconnect()
        ShowNotification("AUTO-BUY BRAINROT KEYBIND", inp.KeyCode.Name)
    end)
end)
...
task.spawn(function()
    ...
    local SENTRY_PROXIMITY_RANGE = 30 -- distance max en studs pour détecter une tourelle

    local function teleportNextToSentry(hrp, targetSentry)
        local root = targetSentry:IsA("BasePart") and targetSentry or targetSentry:FindFirstChildWhichIsA("BasePart", true)
        if root then
            -- Calculer la direction de la caméra horizontale pour se placer EN FACE de la tourelle
            local cam = Workspace.CurrentCamera
            local camLook = cam.CFrame.LookVector
            local flatLook = Vector3.new(camLook.X, 0, camLook.Z)
            if flatLook.Magnitude < 0.1 then flatLook = Vector3.new(0, 0, 1) end
            flatLook = flatLook.Unit
            -- Se placer devant la tourelle (côté face au joueur), à 6 studs
            local spawnPos = root.Position + flatLook * 4 + Vector3.new(0, 3, 0)
            hrp.CFrame = CFrame.new(spawnPos, Vector3.new(root.Position.X, spawnPos.Y, root.Position.Z))
        end
    end

    local function getClosestSentry()
        local _, hrp = getChar()
        local closest, shortestDist = nil, SENTRY_PROXIMITY_RANGE
            ...
            if LocalPlayer:GetAttribute("Stealing") == true then
                ...
                if targetSentry then
                    while targetSentry and targetSentry.Parent and (LocalPlayer:GetAttribute("Stealing") ~= true) do
                        ...
                        teleportNextToSentry(hrp, targetSentry)
                        if bat then
...
task.spawn(function()
    ...
    local AUTO_BUY_COOLDOWN = 0.03

    local function getChar2()
        local char = LocalPlayer.Character or LocalPlayer.CharacterAdded:Wait()
        local hrp = char:WaitForChild("HumanoidRootPart")
        return char, hrp
    end

    local function teleportAbovePart(hrp, part)
    ...
    local function getBestTarget()
        local entries = SharedState.WorldBrainrots
        if not entries or #entries == 0 then return nil end
        local selectedUID = SharedState.AutoBuySelectedUID
        if selectedUID then
            -- Chercher le brainrot sélectionné
            for _, e in ipairs(entries) do
                if e.uid == selectedUID and e.prompt and e.prompt.Parent and e.prompt.Enabled then
                    return e
                end
            end
            -- Sélection disparue → fallback ANY
        end
        -- ANY : prendre le premier prompt valide
        local _, hrp = getChar2()
    ...
    while true do
        task.wait(0.03)
        if not Config.AutoBuyBrainrot then continue end
        if LocalPlayer:GetAttribute("Stealing") == true then continue end

        local now = os.clock()
        if now - lastAutoBuyTime < AUTO_BUY_COOLDOWN then continue end

        local target = getBestTarget()
        if target and target.prompt and target.prompt.Parent and target.prompt.Enabled then
            lastAutoBuyTime = now
            local startTime = os.clock()
            while target.prompt and target.prompt.Parent and target.prompt.Enabled and Config.AutoBuyBrainrot do
                local _, hrp = getChar2()
                -- Suivre le brainrot en continu
                if target.part and target.part.Parent then
                    teleportAbovePart(hrp, target.part)
                end
                local oDur = target.prompt.HoldDuration
                ...
                for _ = 1, 8 do
                    pcall(function()
                        if fireproximityprompt then
                            ...
                            target.prompt:InputBegan(Enum.UserInputType.MouseButton1)
                            task.wait(0.01)
                            target.prompt:InputEnded(Enum.UserInputType.MouseButton1)
                        end
                    end)
                end
                target.prompt.HoldDuration = oDur
                task.wait(0.03)
...
UserInputService.InputBegan:Connect(function(input, gpe)
    if gpe then return end
    if Config.AutoBuyBrainrotKey ~= "" and input.KeyCode == (Enum.KeyCode[Config.AutoBuyBrainrotKey] or Enum.KeyCode.Unknown) then
        Config.AutoBuyBrainrot = not Config.AutoBuyBrainrot
        SaveConfig()
        ShowNotification("AUTO BUY BRAINROT", Config.AutoBuyBrainrot and "ENABLED" or "DISABLED")
    end
...
task.spawn(function()
    ...
    if privateBuild then
        title.Text = "LANCELOTHUB PRIVATE"
    else
        title.Text = "LANCELOTHUB"
    end
    title.Font = Enum.Font.GothamBlack
    ...
    if privateBuild then
        ...
        author.Text = "discord.gg/lancelothub"
    end
